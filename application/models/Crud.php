...
//Start-Read
	...
	public function GetSiswa_Jps($where = "") {
		$data = $this->db->query('select distinct siswa.SW_ID, siswa.SW_NAMA from siswa, detil_rombel_siswa where siswa.SW_ID=detil_rombel_siswa.SW_ID');
		return $data->result_array();
	}
...
//End-Insert
	//join 2 table
	public function Join($table_asal,$table_name,$where,$key,$value){
		$this->db->select('*');
        $this->db->from($table_asal);
        $this->db->join($table_name, $where);
	...
	function getLastPagination($table_name,$key){

     $data = $this->db->query("select * from ".$table_name." order by ".$key." DESC limit 1");
     	if($data->num_rows() > 0) {
            return $data->result_array();
    	}
    ...
    public function TotalRow($table,$atribut,$value)
		{
			$this->db->select($atribut);
			$this->db->where($atribut,$value);
